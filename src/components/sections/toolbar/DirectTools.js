  ...
  const renderChildTool = parent => parent.children.map(child => {
    return (
      ...
      >
        ...
        <span style={{ color: 'white' }} className="ml-3">{child.name} Tool</span>
      </li>
    )
  });
    ...
    if (tool.children) {
      ...
      return (
        <a
          ...
          onClick={() => {
            onClick(tool.id)
            onChildToolClick(tool.id, selectedChild ? selectedChild.id : firstChild.id);
          }}
        ...
        >
          ...
          />
          <i className={selectedChild ? selectedChild.icon : firstChild.icon} style={{ color: 'white' }} />
